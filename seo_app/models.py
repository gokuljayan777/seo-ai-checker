...
class KeywordRanking(models.Model):
    ...
    domain = models.ForeignKey(Domain, on_delete=models.CASCADE, related_name='keyword_rankings')
    keyword = models.ForeignKey(Keyword, on_delete=models.CASCADE, related_name='rankings')
...
class Competitor(models.Model):
    ...
    domain = models.ForeignKey(Domain, on_delete=models.CASCADE, related_name='competitor_of')
    competitor_domain = models.ForeignKey(Domain, on_delete=models.CASCADE, related_name='competing_against')
...
class Backlink(models.Model):
    ...
    target_domain = models.ForeignKey(Domain, on_delete=models.CASCADE, related_name='backlinks')
    source_url = models.URLField()
    source_domain = models.CharField(max_length=255)
    
    anchor_text = models.TextField(blank=True)
    link_type = models.CharField(max_length=20, choices=[('dofollow', 'DoFollow'), ('nofollow', 'NoFollow')], default='dofollow')
    quality = models.CharField(max_length=10, choices=QUALITY_CHOICES, default='medium')  # Based on DA/DR
...
class ContentPage(models.Model):
    ...
    domain = models.ForeignKey(Domain, on_delete=models.CASCADE, related_name='content_pages')
    url = models.URLField()
    ...
    # SEO metrics
    primary_keyword = models.ForeignKey(Keyword, on_delete=models.SET_NULL, null=True, blank=True)
...
class SiteMetrics(models.Model):
    ...
    domain = models.ForeignKey(Domain, on_delete=models.CASCADE, related_name='metrics_history')
...
class AuditHistory(models.Model):
    ...
    page = models.ForeignKey(Page, on_delete=models.CASCADE, related_name='audit_history')
