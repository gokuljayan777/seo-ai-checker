...
def generate_suggestions_from_issues(parsed: dict) -> dict:
    ...
    if isinstance(issues, list):
        for issue in issues:
            if isinstance(issue, dict):
                ...
                issue_codes.append(
                    issue.lower().replace(" ", "_").replace("(", "").replace(")", "")
                )
    ...
    if title_len < 30:
        ...
        suggestions.append(
            "Expand your title to 30-60 characters. Current: {} chars".format(title_len)
        )
    elif title_len > 60:
        improved_title = title[:57] + "..."
        suggestions.append(
            "Shorten your title to under 60 characters for better SERP display. Current: {} chars".format(
                title_len
            )
        )
    ...
    if meta_len == 0:
        improved_meta = (
            title + " - Learn more about this topic in our comprehensive guide."
        )
        suggestions.append(
            "Add a meta description (50-160 chars) to improve CTR in search results"
        )
    elif meta_len < 50:
        improved_meta = meta + " Discover more insights and best practices."
        suggestions.append(
            "Expand meta description to 50+ characters. Current: {} chars".format(
                meta_len
            )
        )
    elif meta_len > 160:
        ...
        suggestions.append(
            "Shorten meta description to under 160 characters. Current: {} chars".format(
                meta_len
            )
        )
    ...
    if h1_count == 0:
        ...
        improved_h1 = h1s[0]
        suggestions.append(
            "Use only one H1 per page. Currently found: {}. Keep the main one, convert others to H2/H3.".format(
                h1_count
            )
        )
    ...
    if word_count < 300:
        suggestions.append(
            "Expand content to at least 300 words (preferably 800+). Current: {} words".format(
                word_count
            )
        )
    elif word_count < 800:
        suggestions.append(
            "Consider expanding to 800+ words for better SEO performance. Current: {} words".format(
                word_count
            )
        )
    ...
    if len(images_without_alt) > 0:
        suggestions.append(
            "Add descriptive alt text to {} images for better accessibility and SEO".format(
                len(images_without_alt)
            )
        )
    if len(images) == 0:
        suggestions.append(
            "Consider adding relevant images to improve engagement and reduce bounce rate"
        )
    ...
    for code in issue_codes:
        if "thin_content" in code and "content length" not in "\n".join(suggestions):
            suggestions.append(
                "Content seems thin. Add more valuable content to improve SEO ranking"
            )
        elif "multiple_h1" in code and "only one H1" not in "\n".join(suggestions):
            suggestions.append(
                "Search engines prefer pages with a single H1. Restructure your heading hierarchy."
            )
    ...
    seo_summary += (
        "Focus on content quality, proper heading structure, and descriptive metadata."
    )
